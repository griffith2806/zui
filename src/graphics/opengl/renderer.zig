...
pub const Renderer = struct {
    ...
    /// `rect` when this backend has a clip primitive, with the offset inverted.
    /// OpenGL currently has no setClip/clearClip, so inner shadows are unclipped.
    pub fn drawShadow(self: *Renderer, rect: Rect, corners: paint.Corners, shadow: paint.Shadow) void {
        ...
        const can_clip = comptime @hasDecl(Renderer, "setClip") and @hasDecl(Renderer, "clearClip");
        if (inner) {
            if (comptime can_clip) {
                self.setClip(rect);
